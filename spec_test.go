...
func TestNext(t *testing.T) {
	runs := []struct {
		...
		{"2012-11-04T03:00:00-0400", "10 20 10 15 7 * 2015", "2015-07-15T10:20:10-0400"},
	}
...
//		log.Printf("%s",reflect.TypeOf(sched).String())

		var actual time.Time
		switch reflect.TypeOf(sched).String() {
		case "*cron.SpecSchedule":
			var n *SpecSchedule
			n,_ = sched.(*SpecSchedule)
			if(n.Year == "*"){
				actual = sched.Next(getTime(c.time))
			}else{
				actual = n.Next(time.Now())
			}
...
//			if ny, err := time.LoadLocation("Local"); err == nil {
				t = t.In(ny)
			}
			log.Printf("%v", t)
		}
